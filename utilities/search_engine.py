...
class OfflineSearchEngine:
    ...
    def query(self, query_str: str, top_k: int = 2, category: str = None) -> str:
        ...
        catalog_result = query_catalog(query_str, top_k=top_k, category=category)
        ...
        core_terms = [t for t in sanitized_terms if (len(t) > 2 or t in COMMAND_TERMS) and t not in stop_words]

        if not core_terms:
            core_terms = [t for t in sanitized_terms if len(t) > 2 or t in COMMAND_TERMS]
        ...
        try:
            with sqlite3.connect(f"file:{self.db_path}?mode=ro", uri=True) as conn:
                ...
                if not cursor.fetchone():
                    return catalog_result or "Search index not initialized."
